...
EOF
  microk8s.kubectl create -f $TEMP_DIR/environment_check.yaml
}

cleanup() {
  echo "cleaning up..."
  microk8s.kubectl delete -f $TEMP_DIR/environment_check.yaml
  rm -rf $TEMP_DIR
...
wait_ds_ready() {
  while true; do
    local ds=$(microk8s.kubectl get ds/longhorn-environment-check -o json)
...
validate_ds() {
  ...
  local pods=$(microk8s.kubectl -l app=longhorn-environment-check get po -o json)
...
dependencies microk8s.kubectl jq mktemp
TEMP_DIR=$(mktemp -d)
trap cleanup EXIT
create_ds
wait_ds_ready
