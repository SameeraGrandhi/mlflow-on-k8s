# Manual Deployment process for additional required service
Once you completed Kubernetes cluster setup, you need to setup the following service to play with mlflow project on Kubernetes cluster.

### 1. Create the Database service on Kubernetes
The back-end consists of a MySQL database & it has a Deployment and a Service. The deployment manages the pods started for mysql service. The Service creates a stable DNS entry for mysql service  so they can reference their dependencies by name.
...
```
#####  Step 2:  Start Mysql Service in a container
...
```
#####  Step 3: Create Database & User to update the mlflow tracking information
...
```
#####  Step 2:  Start Mlflow server Service in a container
